...
```{r}
ca_counties_df <- ca_counties_sf |> as.data.frame() %>% 
  select(-geometry)
```
...
ggplot(ca_counties_sf)+
  geom_sf(aes(fill=land_km2))+
  scale_fill_gradientn(colors=c("cornflowerblue","orchid","red4"))+
  theme_void()
```
...
### Pseudocode
1. set the crs so that they match
2. group by county and summarize by count with n()
3. plot as layers: the points from the sesbania and the layer of counties
...
```{r}
solano <- sesb_counties_sf %>% 
  filter(county == "Solano")

solano_shape <- ca_sesb_sf %>% 
  filter(county == "Solano")

ggplot()+
  geom_sf(data = solano_shape,fill="green")+
  geom_sf(data = solano,color = "blue")+
  theme_void()
